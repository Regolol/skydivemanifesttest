...
```
Browse the directory [layout](src/components/layout) to see which layouts does exist.

If you want to add your own layouts, just add a new component in the directory mentioned above. To start your work, just
copy the content of the [BlankLayout.vue](src/components/layout/BlankLayout.vue). Note that the line
`<slot><router-view></router-view></slot>` is very important to display the routes component. Once you added your
layout, import and register your component in [main.js](src/main.js).

You can also style your layouts separately. Just add a file to the directory [layouts](assets/scss/layouts) and name it
appropriate to your layout. If your layouts name is `DefaultLayout` you should name your stylesheet `_default.scss`. In
the layout component you can then import the stylesheet:
```
<style lang="scss" scoped>
@import "../../assets/scss/layouts/default.scss";
</style>
```
Note: The attribute `scoped` is important to keep the stylesheet separated from other layouts.

## Internationalization
The internationalization support is enabled by default. You'll find all supported languages in the
[src/locales](src/locales) folder. The file [i18n.js](src/i18n.js) provides the `VueI18n` object and a function
`loadLanguageAsync` that lazy loads files. For example, you could import this function to implement a language changer
component.
...
will also be always the fallback language, if the configured default language couldn't be loaded.
...
`/subdir/dist`. If your are using Apache, you have to do the same for the [RewriteBase in the .htaccess file](public/.htaccess#L3).
